...
PSampleConfiguration g_pSampleConfiguration = NULL;
...
STATUS send_frame(PFrame pFrame, UINT32 isVideo) 
{
    ...
    if (NULL ==  g_pSampleConfiguration) {
        return 0;
    }
    PSampleConfiguration pSampleConfiguration = g_pSampleConfiguration;

    MUTEX_LOCK(pSampleConfiguration->streamingSessionListReadLock);
        for (i = 0; i < pSampleConfiguration->streamingSessionCount; ++i) {
            if (isVideo) {
                status = writeFrame(pSampleConfiguration->sampleStreamingSessionList[i]->pVideoRtcRtpTransceiver, pFrame);
                encoderStats.encodeTimeMsec = 4; // update encode time to an arbitrary number to demonstrate stats update
                    updateEncoderStats(pSampleConfiguration->sampleStreamingSessionList[i]->pVideoRtcRtpTransceiver, &encoderStats);
            } else {
                status = writeFrame(pSampleConfiguration->sampleStreamingSessionList[i]->pAudioRtcRtpTransceiver, pFrame);
            }
...
PVOID sendVideoPackets(PVOID args)
{
    ...
    UINT32 maxVideoFrameSize = (256 * 1024);
    UINT32 maxAudioFrameSize = (64 * 1024);
    ...
    // Re-alloc if needed
    if (maxVideoFrameSize > pSampleConfiguration->videoBufferSize) {
        pSampleConfiguration->pVideoFrameBuffer = (PBYTE) MEMREALLOC(pSampleConfiguration->pVideoFrameBuffer, maxVideoFrameSize);
        ...
        pSampleConfiguration->videoBufferSize = maxVideoFrameSize;
    }

    g_pSampleConfiguration = pSampleConfiguration;
    start_fetchstream();
...
{
    while (1) {
        printf ("sending audio packet to peer\n");
        sleep(10000000);
    }
    return NULL;
    STATUS retStatus = STATUS_SUCCESS;
